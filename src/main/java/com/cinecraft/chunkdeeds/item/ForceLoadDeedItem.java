...
public class ForceLoadDeedItem extends Item {
    public ForceLoadDeedItem() {
    ...
    public static ItemStack createStack(int value) {
        ItemStack stack = new ItemStack(ModItems.FORCELOAD_DEED);
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        ...
        tooltip.add(Text.translatable("tooltip.chunkdeeds.grants_forceload", value).formatted(Formatting.AQUA));
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        int maxTotal = config.forceLoadDeed.maxTotalExtra;
        if (maxTotal > 0) {
            int currentExtra = FTBChunksIntegration.getExtraForceLoads(player);
            if (currentExtra + value > maxTotal) {
                player.sendMessage(Text.translatable("message.chunkdeeds.cap_reached_forceload", currentExtra, maxTotal).formatted(Formatting.RED), true);
                return TypedActionResult.fail(stack);
            }
        }

        boolean success = FTBChunksIntegration.addExtraForceLoads(player, value);
        if (success) {
            ServerWorld serverWorld = (ServerWorld) world;

            if (config.general.playSounds) {
                serverWorld.playSound(null, player.getX(), player.getY(), player.getZ(),
                        SoundEvents.BLOCK_BEACON_ACTIVATE, SoundCategory.PLAYERS, 0.75f, 1.4f);
            }
            ...
            int newTotal = FTBChunksIntegration.getExtraForceLoads(player);
            player.sendMessage(Text.translatable("message.chunkdeeds.redeemed_forceload", value, newTotal).formatted(Formatting.AQUA), false);
